    ...
    def score_answers(self, question: Question, answers: List[Answer]) -> float:
        ...
        for i in range(0, len(answers)-1):
            ...
            for ans in answers[:i]:
                if question.get_similarity(answers[i], ans) == 1.0:
                    unique = False

            for ans2 in answers[i+1:]:
                if question.get_similarity(answers[i], ans2) == 1.0:
                    unique = False
        ...
        unique1 = True
        for ans3 in answers[:-1]:
            if question.get_similarity(answers[-1], ans3) == 1.0:
                unique1 = False
        if unique1:
            return 0.0
