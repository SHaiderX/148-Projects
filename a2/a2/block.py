...
class Block:
    ...
    def smash(self) -> bool:
        ...
        self.colour = None
        for i in range(4):
            self.children.append(Block(child_position[i], self._child_size(),
    ...
    def swap(self, direction: int) -> bool:
        ...
        pos0 = child0.position
        child1 = self.children[1]
        pos1 = child1.position
        child2 = self.children[2]
        pos2 = child2.position
        child3 = self.children[3]
        pos3 = child3.position

        if direction == 1:
            # 0, 3 switch          [1 0]
            # 2, 1, switch         [2 3]       [0, 1, 2, 3]
            child0._update_children_positions(pos3)
            child1._update_children_positions(pos2)
            child2._update_children_positions(pos1)
            child3._update_children_positions(pos0)

            self.children = [child3, child2, child1, child0]

        else:  # direction == 0
            # 0, 1 switch          [1 0]
            # 2, 3, switch         [2 3]       [0, 1, 2, 3]
            child0._update_children_positions(pos1)
            child1._update_children_positions(pos0)
            child2._update_children_positions(pos3)
            child3._update_children_positions(pos2)
    ...
    def paint(self, colour: Tuple[int, int, int]) -> bool:
        ...
        """
        if len(self.children) != 0 or self.level != self.max_depth or \
                self.colour == colour:
            return False
    ...
    def combine(self) -> bool:
        ...
        colors = {}
        for child in self.children:
            if child.colour in colors:
                colors[child.colour] = colors[child.colour] + 1
            else:
                colors[child.colour] = 1
